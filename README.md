...
该框架是对象属性的核查框架，通过引入黑白名单和匹配器机制，能够通过可用和不可用两个方面对对象的属性进行匹配，匹配上后黑名单是不可用，而白名单是只可用。所有的拦截都是基于基本的类型，而对于复杂类型（集合、map或者自定义类型）都是由基本类型组成的，框架也是支持对复杂类型进行拆解并核查内部的基本类型进而对复杂类型进行拦截。该框架具有以下特性：
